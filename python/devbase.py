...
sys.path.insert(0, str(MODULES_DIR)) # Imports dos módulos locais
try:
...
def cmd_setup(args, ui: UI, root: Path):
    ...
    modules = [
        ("Core", lambda fs, ui, **kw: run_setup_core(fs, ui, **kw)),
        ("PKM", lambda fs, ui, **kw: run_setup_pkm(fs, ui, **kw)),
        ("Code", lambda fs, ui, **kw: run_setup_code(fs, ui, **kw)),
        ("AI", lambda fs, ui, **kw: run_setup_ai(fs, ui, **kw)),
        ("Operations", lambda fs, ui, **kw: run_setup_operations(fs, ui, **kw)),
        ("Templates", lambda fs, ui, **kw: run_setup_templates(root, fs, ui)),
        ("Hooks", lambda fs, ui, **kw: run_setup_hooks(root, fs, ui)),
    ]
