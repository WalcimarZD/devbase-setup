...
class KnowledgeGraph:
    ...
    def scan(self) -> Dict[str, int]:
        ...
        # Temporary storage for extracted links to avoid re-reading files
        # Map source_rel -> list of extracted links
        pending_links: Dict[str, Tuple[Path, List[str], List[str]]] = {}

        # 1. Single Pass: Collect nodes, build file map, and extract links from content
        for path in search_paths:
            ...
            for file_path in scan_directory(path, extensions={'.md'}):
                ...
                try:
                    ...
                    # Should not happen if file_path is from search_paths which are children of root
                    continue

                content = ""
                # Parse Frontmatter for title/tags
                try:
                    # Bolt Optimization: Read content once via frontmatter.load
                    ...
                    tags = []
                    # Fallback: read content directly if frontmatter failed
                    try:
                ...
                # Extract links immediately (in-memory) to avoid second I/O pass
                if content:
                    md_links = md_link_pattern.findall(content)
                    wiki_links = wiki_link_pattern.findall(content)
                    if md_links or wiki_links:
                        pending_links[rel_path] = (file_path, md_links, wiki_links)

        # 2. Resolution Pass: Process extracted links and add edges (In-Memory)
        for source_rel, (file_path, md_links, wiki_links) in pending_links.items():
