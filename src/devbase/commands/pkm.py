...
def find(
    ...
    for result in results:
        ...
        if query:
            title_text.highlight_regex(f"(?i){re.escape(query)}", style="bold black on yellow")
        ...
        if result['content_preview']:
            ...
            preview_text = Text(f"{preview}...", style="dim")

            if query:
                preview_text.highlight_regex(f"(?i){re.escape(query)}", style="bold black on yellow")
