...
def _verify_dependencies(root: Path, report: Dict[str, List[str]]):
    ...
    try:
        ...
        ignore_libs = ["toml", "jinja2", "shellingham", "python-frontmatter", "copier", "typer", "rich", "fastembed", "groq"]
...
def _verify_cli_consistency(root: Path, report: Dict[str, List[str]], fix: bool):
    ...
    for module, cmds in found_commands.items():
        for cmd in cmds:
            ...
            # Also check for aliases or different structures
            # We look for `devbase module cmd` or just command mention in code block

            # Simple check
            if f"devbase {module} {cmd}" not in usage_content and f"`{cmd}`" not in usage_content:
    ...
    if missing_docs:
        ...
        if fix and usage_guide.exists():
            ...
            for cmd in missing_docs:
                cmd_str = f"- `devbase {cmd}`"
                if cmd_str not in content:
                    new_lines.append(f"{cmd_str}\n")
                    added_count += 1

            if added_count > 0:
                with open(usage_guide, "a") as f:
                    f.writelines(new_lines)
                report["updated"].append(f"USAGE-GUIDE.md (added {added_count} undocumented commands)")
...
def _verify_db_integrity(root: Path, report: Dict[str, List[str]]):
    ...
    target_tables = ["hot_fts", "cold_fts"]

    # 1. Check Definitions in Adapter
    if adapter_file.exists():
        adapter_content = adapter_file.read_text()
        for table in target_tables:
            # Look for CREATE TABLE ... table_name
            if not re.search(f"CREATE TABLE.*{table}", adapter_content, re.IGNORECASE | re.DOTALL):
                report["warnings"].append(f"Table '{table}' not defined in duckdb_adapter.py")
    else:
        report["warnings"].append("duckdb_adapter.py not found.")

    # 2. Check Documentation
    for table in target_tables:
        if table not in tech_content:
            report["warnings"].append(f"Table '{table}' missing in TECHNICAL_DESIGN_DOC.md")
...
def _check_changelog(root: Path, report: Dict[str, List[str]], changes: List[str], fix: bool):
    ...
    if "[Unreleased]" not in content and "In Progress" not in content:
        ...
        if fix:
            ...
            # Insert after the title or at top
            lines = content.splitlines()
            insert_idx = 0
            # Try to find the first version header "## [..."
            for i, line in enumerate(lines):
                if line.startswith("## [") or line.startswith("## v"):
                    insert_idx = i
                    break

            # If no headers found, append to end? No, probably prepend after title
            if insert_idx == 0:
                # Find title
                for i, line in enumerate(lines):
                    if line.startswith("# "):
                        insert_idx = i + 2 # Skip title and maybe a blank line
                        break
