...
## 2026-01-23 - [PKM Find Highlighting]
**Learning:** In search interfaces, users rely heavily on scanning for context. Displaying raw text snippets without highlighting the search term forces the user to read carefully to find relevance. Adding high-contrast highlighting (e.g., black on yellow) significantly improves scanning speed and perceived system intelligence.
**Action:** When implementing search features, always use `rich.text.Text.highlight_regex` to visually emphasize the query in the results.
