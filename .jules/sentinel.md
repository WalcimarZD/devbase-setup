...
## 2025-01-03 - Command Injection in PKM Editor Launch
**Vulnerability:** `pkm journal` and `pkm icebox` commands used `subprocess.run(f"code {file_path}", shell=True)` to open files in VS Code. This allowed command injection if the file path contained shell metacharacters (e.g. `; rm -rf /`).
**Learning:** Using `shell=True` with formatted strings is almost always a vulnerability when handling paths or user input. Even if paths seem controlled (e.g. date-based filenames), the root path might be user-controlled via `--root`.
**Prevention:** Always use the list format for `subprocess.run` (e.g. `["code", str(path)]`) and avoid `shell=True`. This handles spaces and special characters safely without invoking the shell.
